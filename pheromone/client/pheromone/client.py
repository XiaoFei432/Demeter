    ...
    def call_app(self, app_name, func_args, synchronous=False, timeout=5000, retry=0):
        ...
        for item in func_args:
            ...
            if len(item) == 2:
                ...
                func, args, resources = item
                cpu_cores = resources.get('cpu_cores')
                memory_mb = resources.get('memory_mb')
            else:
                raise ValueError('func_args items must be (function, args) or (function, args, resources)')
            func_req = call.requests.add()
            func_req.name = func
            if cpu_cores is not None:
                func_req.cpu_cores = float(cpu_cores)
            if memory_mb is not None:
                func_req.memory_mb = int(memory_mb)
            for arg in args:
