...
void schedule_func_call(logger log, CommHelperInterface *helper, map<uint8_t, uint8_t> &executor_status_map, map<string, set<uint8_t>> &function_executor_map,
                        ...
                        double cpu_cores = 0.0, unsigned memory_mb = 0){
  ...
  if (avail_warm_executors.size() > 0 || avail_cold_executors.size() > 0){
    ...
    string resource_meta = std::to_string(cpu_cores) + "," + std::to_string(memory_mb);
    if (resource_meta.size() > 255) {
      resource_meta = "0,0";
    }
    resp.push_back(static_cast<uint8_t>(resource_meta.size()));
    resp += resource_meta;
    resp += session_id + "|";
    ...
    if (schedDelayTime > 0){
      delay_call_queue.push(DelayFunctionCall{session_id, app_name, func_name, func_args, arg_flag, cpu_cores, memory_mb, std::chrono::system_clock::now()});
    }
    else {
      if (!rejectExtraReq) {
    ...
    for (auto &comm_resp : comm_resps) {
      if (comm_resp.msg_type_ == RecvMsgType::Call) {
        ...
        for (int i = 0; i < comm_resp.func_name_.size(); i++){
          ...
          double cpu_cores = i < comm_resp.cpu_cores_.size() ? comm_resp.cpu_cores_[i] : 0.0;
          unsigned memory_mb = i < comm_resp.memory_mb_.size() ? comm_resp.memory_mb_[i] : 0;
          if (is_func_arg_key > 0) {
            ...
            inflight_args.cpu_cores_ = cpu_cores;
            inflight_args.memory_mb_ = memory_mb;
            for (auto &arg : func_args){
            ...
            if (inflight_args.inflight_args_.empty()){
              ...
              schedule_func_call(log, helper, executor_status_map, function_executor_map, comm_resp.session_id_, comm_resp.app_name_,
                                 inflight_args.func_name_, func_full_args, is_func_arg_key, cpu_cores, memory_mb);
            }
            else{
            ...
            schedule_func_call(log, helper, executor_status_map, function_executor_map, comm_resp.session_id_, comm_resp.app_name_,
                               func_name, func_args, 0, cpu_cores, memory_mb);
          }
          call_id++;
        }
      }
          ...
          for (auto cid : key_call_id_map[comm_resp.data_key_]){
            ...
            if (call_id_inflight_args_map[cid].inflight_args_.empty()){
              ...
              schedule_func_call(log, helper, executor_status_map, function_executor_map,
                                ...
                                call_id_inflight_args_map[cid].cpu_cores_,
                                call_id_inflight_args_map[cid].memory_mb_);
    ...
    if (!delay_call_queue.empty()){
      ...
      while (cur_avail_executors > 0 && !delay_call_queue.empty()) {
        ...
        schedule_func_call(log, helper, executor_status_map, function_executor_map, delay_call.session_id_, 
                            ...
                            delay_call.cpu_cores_, delay_call.memory_mb_);
        cur_avail_executors--;
