  ...
  while (true){
    ...
    if (str != nullptr) {
      ...
      if (str[0] == 1){
        ...
        uint8_t resource_meta_len = static_cast<uint8_t>(str[3]);
        string resource_meta(str + 4, resource_meta_len);
        vector<string> resource_parts;
        split(resource_meta, ',', resource_parts);
        if (resource_parts.size() == 2) {
          setenv("DEMETER_CPU_CORES", resource_parts[0].c_str(), 1);
          setenv("DEMETER_MEMORY_MB", resource_parts[1].c_str(), 1);
        }

        string msg(str + 4 + resource_meta_len);
