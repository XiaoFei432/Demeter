...
void func_call_handler(logger log, string &serialized, SocketCache &pushers,
                        ...
                        const DemeterSchedulerConfig &demeter_config){
  ...
  if (demeter_config.enabled_) {
    scheduled_node_msg = demeter_schedule_function_call(
        log, call_msg, serialized, source, node_status_map, demeter_config, seed);
  }

  // batch scheduling
  if (scheduled_node_msg.empty() && req_num > 1) {
  ...
  else if (scheduled_node_msg.empty()) {
    for (auto &pair : node_status_map){
      if (!source.empty() && source == pair.first){
        continue;
      }
