...
DemeterSchedulerConfig demeterSchedulerConfig;
...
void run(Address public_ip, Address private_ip, unsigned thread_id) {
  ...
  while (true) {
    ...
    if (pollitems[7].revents & ZMQ_POLLIN) {
      ...
      func_call_handler(log, serialized, pushers, node_status_map, demeterSchedulerConfig);
      call_count++;
    }
...
int main(int argc, char *argv[]) {
  ...
  demeterSchedulerConfig = load_demeter_scheduler_config(conf);
