...
gen_demeter_nodes() {
  ...
  for NODE in "${ARR[@]}"; do
    ...
    IP_ADDR="${PARTS[0]}"
    DC_NAME="${PARTS[1]:-${PARTS[0]}}"
    RESULT=$"$RESULT    - ip: $IP_ADDR\n"
    RESULT=$"$RESULT      dc: $DC_NAME\n"
    RESULT=$"$RESULT      warm_weight: ${DEMETER_WARM_WEIGHT:-0.15}\n"
    RESULT=$"$RESULT      load_weight: ${DEMETER_LOAD_WEIGHT:-0.35}\n"
    RESULT=$"$RESULT      locality_weight: ${DEMETER_LOCALITY_WEIGHT:-1.0}\n"
  done
...
if [[ "$ROLE" = "coordinator" ]]; then
  ...
  if [[ "${DEMETER_ENABLED:-0}" = "1" ]]; then
    echo -e "demeter:" | tee -a conf/config.yml
    echo -e "  enabled: 1" | tee -a conf/config.yml
    echo -e "  memory_loss_factor: ${DEMETER_MEMORY_LOSS_FACTOR:-0.8}" | tee -a conf/config.yml
    echo -e "  nodes:" | tee -a conf/config.yml
    if [[ -n "$DEMETER_NODES" ]]; then
      gen_demeter_nodes "$DEMETER_NODES" | tee -a conf/config.yml
    fi
  fi
